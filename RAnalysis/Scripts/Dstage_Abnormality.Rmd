...
aovMOD_Exp1_means_contr <- emmeans::emmeans(object = aovMOD_Exp1, # run tukey
                                    ...
                                    adjust = "tukey")
aovMOD_Exp1_mod_means   <- multcomp::cld(object = aovMOD_Exp1_means_contr$emmeans, Letters = letters) # letter display\
...
aovMOD_Exp1_LowvMod <- lm(Percent.deformed ~ Larvae_pCO2, 
                          data= (df_Exp1 %>% dplyr::filter(!Larvae_pCO2 %in% 'High pCO2')))
shapiro.test(resid(aovMOD_Exp1_LowvMod)) # 0.2501 - norm
leveneTest(aovMOD_Exp1_LowvMod) # 0.2707- pass
summary(aov(aovMOD_Exp1_LowvMod))
#             Df   Sum Sq  Mean Sq F value Pr(>F)
# Larvae_pCO2  1 0.003486 0.003486   1.198  0.316
# Residuals    6 0.017456 0.002909

# t test
t_test(Percent.deformed ~ Larvae_pCO2, 
                                 data= (df_Exp1 %>% dplyr::filter(!Larvae_pCO2 %in% 'High pCO2')))
# 1	Percent.deformed	Low pCO2	Moderate pCO2	4	4	-1.094656	3.846246	0.337
df_Exp1 %>%
  dplyr::filter(!Larvae_pCO2 %in% 'High pCO2') %>% 
  t_test(data =., Percent.deformed ~ Larvae_pCO2) %>%
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj")
# Percent.deformed	Low pCO2	Moderate pCO2	4	4	-1.094656	3.846246	0.337	0.337	


capture.output(aovMOD_Exp1_LowvMod,file="Output/2_Dstage_Shell_Abnormalities/Experiment1_ANOVA_LowvMOD.doc") 

```

```{r  Statistics; F1 Experiment #2} 
