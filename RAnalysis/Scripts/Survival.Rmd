...
binary_df_F1_F2_all <- rbind(binary_df_F1_all, binary_df_F2_all) %>% dplyr::filter(!(Larvae_pCO2 == 'High pCO2'))
binary_df_F1_F2_all$Larvae_pCO2 <- factor(binary_df_F1_F2_all$Larvae_pCO2, exclude = 'High pCO2')
unique(binary_df_F1_F2_all$Larvae_pCO2)
#  by mean tank
binary_df_F1_F2_MEANS <- rbind(binary_df_F1_MEANS, binary_df_F2_MEANS) %>% dplyr::filter(!(Larvae_pCO2 == 'High pCO2'))
binary_df_F1_F2_MEANS$Larvae_pCO2 <- factor(binary_df_F1_F2_MEANS$Larvae_pCO2, exclude = 'High pCO2')
unique(binary_df_F1_F2_MEANS$Larvae_pCO2)
```
...
# all
df_F1_all_larvae    <- binary_df_F1_all %>% filter(Age <=10) # F1 
df_F2_all_larvae    <- binary_df_F2_all %>% filter(Age <=10) # F2
df_F1_F2_all_larvae <- binary_df_F1_F2_all  %>% filter(Age <=10) # merged
# mean data by tank replicate within treatment
df_F1_MEANS_larvae    <- binary_df_F1_MEANS %>% filter(Age <=10) # F1 
df_F2_MEANS_larvae    <- binary_df_F2_MEANS %>% filter(Age <=10) # F2
df_F1_F2_MEANS_larvae <- binary_df_F1_F2_MEANS  %>% filter(Age <=10) # merged

# what days are included?
unique(df_F1_all_larvae$Age) # 2  4  8 10
unique(df_F2_all_larvae$Age) # 2  6  8 10
unique(df_F1_F2_all_larvae$Age) #  2  4  8 10  6

unique(df_F1_MEANS_larvae$Age) # 2  4  8 10
unique(df_F2_MEANS_larvae$Age) # 2  6  8 10
unique(df_F1_F2_MEANS_larvae$Age) #  2  4  8 10  6
```
...
# Larvae_pCO2=Low pCO2      1340     1154     1214       3.0      9.98
# Larvae_pCO2=Moderate pCO2 1127      989     1111      13.4     43.15
# Larvae_pCO2=High pCO2      805      762      579      57.5    126.89
...
#                                            N Observed Expected (O-E)^2/E (O-E)^2/V
# Larvae_pCO2=Low pCO2, Generation=1      1340     1154     1068  6.87e+00  2.07e+01
# Larvae_pCO2=Low pCO2, Generation=2       280      199      222  2.44e+00  4.67e+00
# Larvae_pCO2=Moderate pCO2, Generation=1 1127      989      988  2.61e-04  7.55e-04
# Larvae_pCO2=Moderate pCO2, Generation=2  403      268      331  1.20e+01  2.43e+01
```
...
F1_coxph_test_table <- coxph(Surv(Age, Count_dead) ~ Larvae_pCO2+(1|Tank), data = df_F1_all_larvae) %>% 
...
F1_F2_coxph_test <- coxph(formula = Surv(Age, Count_dead) ~ Larvae_pCO2*Generation+(1|Tank), data = df_F1_F2_all_larvae)
F1_F2_coxph_test_table <- coxph(Surv(Age, Count_dead) ~ Larvae_pCO2*Generation+(1|Tank), data = df_F1_F2_all_larvae) %>% 
                              tbl_regression(exp = TRUE) 
#                                         coef exp(coef) se(coef)      z        p
