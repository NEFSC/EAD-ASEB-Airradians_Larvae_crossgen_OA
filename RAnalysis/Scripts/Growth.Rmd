...
```{r  RM Anova length; F1 Experiment #2 low moderate high}
...
lme_Exp2 = lme(Length ~ Larvae_pCO2 * Age, data=Exp2_RMAnova, random = ~1|ID)
anova(lme_Exp2)
...
summary(glht(lme_Exp2, linfct=mcp(Larvae_pCO2 = "Tukey")), 
        test = adjusted(type = "bonferroni"))# low is not different frommoderate 
summary(glht(lme_Exp2, linfct=mcp(Age = "Tukey")), 
        test = adjusted(type = "bonferroni")) # all timepoints differ significnatly
...
# Effect of treatment within each timepoint
...
# NOTE: 1 from low, 6 from moderate adn 15 from high, 6 and 15 only present once through all datapoints, 
# whereas low had all tanks throughout, omititing 2, 3, 4, and 5 (tank IDs) result in 
# low not significant from moderate  in all dates, but low and moderate differencet from high. 
pwc <- Exp2_RMAnova %>%
  dplyr::filter(!(ID %in% c(5, 6,15))) %>% # 6 and 15 were only present once,
  group_by(Age) %>%
  pairwise_t_test(
...
```{r  RM Anova length; F1 Experiment #2 low moderate ONLY}
...
# RUN STATISTICAL MODEL  ::::::::::::::::::::::::::::::::::
df_Exp2_res.aov <- anova_test(data = Exp2_RMAnova_LvM, 
                              formula = Length ~ Age*Larvae_pCO2 + Error(ID/Age))
get_anova_table(df_Exp2_res.aov)
#            Effect  DFn   DFd       F        p p<.05   ges
...
lme_Exp2 = lme(Length ~ Larvae_pCO2 * Age, data=Exp2_RMAnova_LvM, random = ~1|ID)
anova(lme_Exp2)
...
# POSTHOC TEST ::::::::::::::::::::::::::::::::::
Exp2_RMAnova_LvM$Length <- as.numeric(Exp2_RMAnova_LvM$Length)
Exp2_RMAnova_LvM        <- ungroup(Exp2_RMAnova_LvM)

Exp2_RMAnova_LvM %>%
  group_by(Age) %>%
  levene_test(Length ~ Larvae_pCO2)
# 2	1	7	0.24378413	0.6365998
# 4	1	7	0.03130925	0.8645631
# 8	1	8	0.22119004	0.6506897
# 10	1	8	0.21887714	0.6523773

Exp2_RMAnova_LvM %>%
  group_by(Age) %>%
  shapiro_test(Length)
# 2	Length	0.8698218	0.1224141	
# 4	Length	0.9169024	0.3672120	
# 8	Length	0.9437437	0.5953709	
# 10	Length	0.9692177	0.8834981	
...
lme_Exp4 = lme(Length ~ Larvae_pCO2 * Age, data=Exp4_RMAnova, random = ~1|ID)
anova(lme_Exp4)
# Larvae_pCO2   	1	4	  11.13	 0.0289
# Age	            6	24	545.06 <.0001
# Larvae_pCO2:Age	6	24	1.26	 0.3134

summary(glht(lme_Exp4, linfct=mcp(Age = "Tukey")), 
        test = adjusted(type = "bonferroni"))# low is not different frommoderate 



# POSTHOC TEST ::::::::::::::::::::::::::::::::::
Exp4_RMAnova$Length <- as.numeric(Exp4_RMAnova$Length)
Exp4_RMAnova        <- ungroup(Exp4_RMAnova)


# Effect of time at each level of treatment
one.way2 <- Exp4_RMAnova %>%
...
Exp4_RMAnova %>%
  group_by(Age) %>%
  levene_test(Length ~ Larvae_pCO2)
# 2	1	4	0.002423916	0.9630937
# 4	1	4	0.011976048	0.9181278
# 6	1	4	0.008264463	0.9319353
# 8	1	4	0.769961977	0.4297721
# 10	1	4	0.159425890	0.7100857
# 12	1	4	0.415898618	0.5541153
# 14	1	4	0.155470250	0.7134802
Exp4_RMAnova %>%
  group_by(Age) %>%
  shapiro_test(Length)
# 2	Length	0.7885047	0.04620376	
# 4	Length	0.9810873	0.95683312	
# 6	Length	0.9626058	0.83958108	
# 8	Length	0.9339472	0.61091020	
# 10	Length	0.8628816	0.19927339	
# 12	Length	0.8942218	0.34090174	
# 14	Length	0.9367272	0.63295567	


# Effect of treatment within each timepoint
one.way <- Exp4_RMAnova %>%
                dplyr::group_by(Age) %>%
                rstatix::anova_test(dv = Length, wid = ID, between = Larvae_pCO2) %>%
                rstatix::get_anova_table() %>%
                adjust_pvalue(method = "bonferroni")
