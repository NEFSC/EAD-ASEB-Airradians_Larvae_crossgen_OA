...
Exp2_Exp4_hazardratio.int <- ggforest(Exp2_Exp4_coxph_test, data = binary_Exp2_Exp4_all)
...
Exp2_Pre.calc <- Exp2_PrePost %>% 
                      dplyr::select(c(Age, proportion, Tank, Larvae_pCO2,Generation)) %>% 
...
shapiro.test(resid(TwoWay.Pre)) # 0.6349 - pass
leveneTest(TwoWay.Pre) # 0.7313 - pass
summary(TwoWay.Pre)

#                         Df  Sum Sq  Mean Sq F value Pr(>F)  
# Larvae_pCO2             1 0.00181 0.001806   0.563 0.4676  
# Generation              1 0.01276 0.012760   3.977 0.0694 .
# Larvae_pCO2:Generation  1 0.00077 0.000770   0.240 0.6330  
# Residuals              12 0.03851 0.00320
...
shapiro.test(resid(TwoWay.Post)) # 0.03677 - non-normal
leveneTest(TwoWay.Post) # 0.2298 - pass
# * run non-parametric
SRH.Post <- scheirerRayHare(abs.delta.perc ~ Larvae_pCO2 * Generation, 
                     data = (Exp2Exp4_PrePost.master %>% dplyr::filter(type %in% 'post')))
#                        Df  Sum Sq      H  p.value
# Larvae_pCO2             1 100.000 4.4444 0.035015 **
# Generation              1  26.667 1.1852 0.276303
# Larvae_pCO2:Generation  1  52.267 2.3230 0.127477
# Residuals              12 158.567 
...
Exp2Exp4_PrePost.master  %>% 
  ggplot(aes(y=abs.delta.perc, x = Larvae_pCO2)) +
  geom_bar() + 
