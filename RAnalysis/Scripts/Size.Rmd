...
# Larvae_pCO2        2  11243  5621.7  78.477
# Age                3  44134 14711.4 205.366
# Larvae_pCO2:Age    6   4788   798.0  11.140
...
# Response: Length
#                     Chisq Df Pr(>Chisq)    
# (Intercept)     4486.9890  1    < 2e-16 ***
# Larvae_pCO2        0.9848  1    0.32102    
# Age             2880.1057  6    < 2e-16 ***
# Larvae_pCO2:Age   11.7444  6    0.06792 .  
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
anova(Exp4_LMM.lme4)
#                 npar Sum Sq Mean Sq  F value
# Larvae_pCO2        1    166   165.9  11.1334
# Age                6  48733  8122.2 545.0616
# Larvae_pCO2:Age    6    112    18.7   1.2575
...
# Age lsmean   SE   df lower.CL upper.CL .group 
#  2      102 1.68 25.9     98.6      105  a     
#  4      109 1.68 25.9    105.2      112  a     
#  6      129 1.68 25.9    125.5      132   b    
#  8      143 1.68 25.9    139.2      146    c   
#  10     166 1.68 25.9    162.9      170     d  
#  12     181 1.68 25.9    177.1      184      e 
#  14     200 1.68 25.9    196.3      203       f
lsm_Exp4.LMM.OA <- lsmeans(Exp4_LMM.lme4,list(pairwise~Larvae_pCO2),adjust="tukey")
cld(lsm_Exp4.LMM.OA, Letters=letters)
# Larvae_pCO2   lsmean   SE df lower.CL upper.CL .group
#  Moderate pCO2    144 1.16  4      141      147  a    
#  Low pCO2         150 1.16  4      146      153   b   
...
# * run one way anova 
Exp2_AOV_res.mod <- lm(Length~Larvae_pCO2,data=Exp2_Dstage_LvM)

# * check model assumptions 
shapiro.test(resid(Exp2_AOV_res.mod)) # 0.1038
leveneTest(Exp2_AOV_res.mod) # 0.6366

# * print the summary aov
summary(aov(Exp2_AOV_res.mod))
#             Df Sum Sq Mean Sq F value Pr(>F)  
# Larvae_pCO2  1  10.65  10.653   4.861 0.0633 .
# Residuals    7  15.34   2.192
...
Exp4_LvM <-  Exp4 %>%
                            dplyr::filter(Age %in% c(2,4,8,10,14)) %>% 
                            # dplyr::filter(!Larvae_pCO2 %in% 'High pCO2') %>% # omit high run for low vs. mod
                            dplyr::select(ID, Larvae_pCO2, Age,Length) %>% 
                            convert_as_factor(ID, Age, Larvae_pCO2) %>% 
...
Exp4_LvM_14DPF <- Exp4_LvM %>%  dplyr::filter(Age %in% 14)
...
```{r Two-way tests to comapre size; First metamorphosis at 10 dpf F1s and at 14 dpf F2s}
...
master_metamorphosis <- rbind(Exp2_LvM_10DPF, Exp4_LvM_14DPF)

TWanova <- lm(Length ~ Larvae_pCO2 * Generation, data= master_metamorphosis)
shapiro_test(resid(TWanova)) # 0.5986453 pass
levene_test(TWanova) # 0.3201007 pass
TWanova_summ <- summary( aov (TWanova))
#                        Df Sum Sq Mean Sq F value   Pr(>F)    
# Larvae_pCO2             1  484.3   484.3  10.703  0.00668 ** 
# Generation              1 2146.2  2146.2  47.427 1.68e-05 ***
# Larvae_pCO2:Generation  1   26.2    26.2   0.579  0.46144    
# Residuals              12  543.0    45.3 

```



```{r Two-way tests to comapre size; 14 dpf}
# IMPORTNALTY this is four days AFTER first metamorphosis for F1s and AT FIRST metamorhposis for F2s
#   * data: Exp2_LvM_10DPF and Exp4_LvM_14DPF
master_14DPF <- rbind(Exp2_LvM_14DPF, Exp4_LvM_14DPF)

TWanova <- lm(Length ~ Larvae_pCO2 * Generation, data= master_14DPF)
shapiro_test(resid(TWanova)) # 0.1699191 pass
levene_test(TWanova) # 0.2657175 pass
TWanova_summ <- summary( aov (TWanova))
#                        Df Sum Sq Mean Sq F value   Pr(>F)    
# Larvae_pCO2             1    854     854   1.442    0.253    
# Generation              1  42459   42459  71.699 2.09e-06 ***
# Larvae_pCO2:Generation  1    115     115   0.193    0.668    
# Residuals              12   7106     592   


master_14DPF.summ <- master_14DPF %>%
                      dplyr::select(Larvae_pCO2, Generation, Length) %>% 
                      na.omit() %>% 
                      Rmisc::summarySE(measurevar="Length", groupvars="Generation")
#     n   mean    sd     se     ci
# F1	10	306.157	29.742119	9.405284	21.276230
# F2	6	199.750	4.755944	1.941606	4.991057

master_14DPF.summ <- master_14DPF %>%
                      dplyr::select(Larvae_pCO2, Generation, Length) %>% 
                      na.omit() %>% 
                      Rmisc::summarySE(measurevar="Length", groupvars=c("Generation", "Larvae_pCO2"))
# F1	Low pCO2	5	315.534	27.041934	12.093520	33.576996
# F1	Moderate pCO2	5	296.780	32.237387	14.416998	40.028003
# F2	Low pCO2	3	203.600	2.783882	1.607275	6.915547
# F2	Moderate pCO2	3	195.900	2.080865	1.201388	5.169156
```
