...
### Load Data as df
```{r}
#Here we are assessing our data frame the name "df" so that we do not have to type out the full file name every time we want to call it.
...
# number of runs 
unique(df$Run) # 1,2,3,4,6 - what are these? - Run = the different larval experiments, Runs 1-2 = larvae experiments, Runs 3-5 = F2 larvae experiments, and Run 6 = F3 larval experiments. Runs 2, 4, and 6 went on to be raised in the flow through system. 
...
#single parentage experiments
df_Run1 <- df %>% filter(Run=="1") # Gen 1 run 1 - summarized above, 
df_Run2 <- df %>% filter(Run=="2") # Gen 1 run 2 - summarized above, 
df_Run4 <- df %>% filter(Run=="4") # Gen 2 run 2 - summarized above, 
```

# Plot data as barplots mean +- SE for each run
```{r average and barplots}

# Summarize Percent Deformities for plotting 
...
# dataframes filtered for history 
...
#posthoc
aovMOD_Gen2_LOW_posthoc <- emmeans(aovMOD_Gen2_MOD, pairwise~Larvae_pCO2, adjust="tukey")
multcomp::cld(aovMOD_Gen2_LOW_posthoc$emmeans,alpha = 0.5, Letters = letters)
...
#posthoc
aovMOD_Gen2_MOD_posthoc <- emmeans(aovMOD_Gen2_MOD, pairwise~Larvae_pCO2, adjust="tukey")
multcomp::cld(aovMOD_Gen2_MOD_posthoc$emmeans,alpha = 0.5, Letters = letters)

aovKW_Gen2_HIGH
# Kruskal-Wallis chi-squared = 5.6, df = 2, p-value = 0.06081
dunnTest(Percent.deformed ~ Larvae_pCO2,
                data = df_Gen2_HIGHhist,
              method="bh")
...
#            Df   Sum Sq  Mean Sq F value Pr(>F)  
#Generation   1 0.003481 0.003481   4.174 0.0683 .
#Residuals   10 0.008340 0.000834 
...
#            Df  Sum Sq  Mean Sq F value Pr(>F)
#Generation   1 0.00452 0.004517   0.994  0.345
#Residuals    9 0.04088 0.004542  
...
ggplot(data=df_MS_1, aes(x=Larvae_pCO2, y=Percent.deformed, fill=Larvae_pCO2)) +
  ...
  facet_wrap("Run")
```
# Comparison among generations within history
```{r } 
...
```
# Stats for runs with single parentage
```{r}
aovMOD_Run1 <- lm(Percent.deformed ~ Larvae_pCO2, data=df_Run1)
shapiro.test(resid(aovMOD_Run1)) # 0.4186 - norm
leveneTest(aovMOD_Run1) # 0.4478- pass
summary(aov(aovMOD_Run1))
aovMOD_Run1_posthoc <- emmeans(aovMOD_Run1, pairwise~Larvae_pCO2, adjust="tukey")
multcomp::cld(aovMOD_Run1_posthoc$emmeans,alpha = 0.5, Letters = letters)

aovMOD_Run2 <- lm(Percent.deformed ~ Larvae_pCO2, data=df_Run2)
shapiro.test(resid(aovMOD_Run2)) # 0.2499 - norm
leveneTest(aovMOD_Run2) # 0.4246- pass
summary(aov(aovMOD_Run2))
aovMOD_Run2_posthoc <- emmeans(aovMOD_Run2, pairwise~Larvae_pCO2, adjust="tukey")
multcomp::cld(aovMOD_Run2_posthoc$emmeans,alpha = 0.5, Letters = letters)

aovMOD_Run4 <- lm(Percent.deformed ~ Larvae_pCO2, data=df_Run4)
shapiro.test(resid(aovMOD_Run4)) # 0.2875 - norm
leveneTest(aovMOD_Run4) # 0.5552- pass
summary(aov(aovMOD_Run4))
aovMOD_Run4_posthoc <- emmeans(aovMOD_Run4, pairwise~Larvae_pCO2, adjust="tukey")
multcomp::cld(aovMOD_Run4_posthoc$emmeans,alpha = 0.5, Letters = letters)

```


# Stats across genrations
Data did not meet the assumptions of normality so the non parametric Scheirer Ray Hare test was used to test for significance among treatments and parental exposures.
```{r}
scheirerRayHare(Percent.deformed ~ Larvae_pCO2*Run,
                data = df)

scheirerRayHare(Percent.deformed ~ Larvae_pCO2*Parent_pCO2,
                data = df)

scheirerRayHare(Percent.deformed ~ Run*Parent_pCO2,
                data = df)
...
```{r}
df_low <- df%>%
  filter(Parent_pCO2 %in% c("Low pCO2"))
...
df_low_low <- df_low%>%
  filter(Larvae_pCO2 %in% c("Low pCO2"))
...
```{r}
sum_mod %>%
   kbl(caption = "ANOVA - Low OA exposures only")%>%
...
leveneTest(Percent.deformed_trans ~ Larvae_pCO2, data=df_run1)
...
```{r}
leveneTest(Percent.deformed_trans ~ Larvae_pCO2, data=df_2021)
...
ggplot(data=df_2022_3, aes(x=Larvae_pCO2, y=Percent.deformed, fill=Treatment)) +
  geom_boxplot()+  scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
...
```{r}
leveneTest(Percent.deformed_trans ~ Larvae_pCO2, data=df_2022_3)
...
dunnTest(Percent.deformed ~ Parent_pCO2,
                data = df_2022_3,
              method="bh")
```
...
ggplot(data=df_2022_4, aes(x=Larvae_pCO2, y=Percent.deformed, fill=Larvae_pCO2)) +
  geom_boxplot()+  scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
...
leveneTest(Percent.deformed_trans ~ Larvae_pCO2, data=df_2022_4)
shapiro.test(df_2022_4$Percent.deformed_trans)
```

```{r}

res.kruskal <- kruskal.test(Percent.deformed ~ Larvae_pCO2, data = df_2022_4)
res.kruskal
...
ggplot(data=df_6, aes(x=Larvae_pCO2, y=Percent.deformed, fill=Larvae_pCO2)) +
  geom_boxplot()+  scale_fill_manual(values=c("forestgreen","orange", "purple"))+
  theme_bw() +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
...
Run6_mean_deformaties <- df_6 %>% 
                dplyr::select(Larvae_pCO2,Parent_pCO2,Percent.deformed) %>% 
                na.omit() %>% 
                dplyr::group_by(Larvae_pCO2,Parent_pCO2) %>% 
                dplyr::summarise(mean_Perc_def = mean(Percent.deformed), 
                                 n           = n(),
                                 sd_Perc_def   = sd(Percent.deformed),
                                 se_Perc_def   = sd_Perc_def/(sqrt(n)))
...
```{r}
leveneTest(Percent.deformed_trans ~ Larvae_pCO2, data=df_6)
shapiro.test(df_6$Percent.deformed_trans)
```

```{r}
mod <- aov(Percent.deformed_trans ~Larvae_pCO2 * Parent_pCO2, data = df_6)
summary(mod)
```
