...
Rep_Summary_R1 <- Rep_Summary_all %>%  dplyr::filter(Run %in% 1) # for the other paper - full grow out multigenerational
Rep_Summary_R1 %>%  kbl() %>% kable_classic(full_width = F, html_font = "Cambria")

Rep_Summary_R2 <- Rep_Summary_all %>%  dplyr::filter(Run %in% 2) # for the other paper - full grow out multigenerational
Rep_Summary_R2 %>%  kbl() %>% kable_classic(full_width = F, html_font = "Cambria")


Rep_Summary_R3 <- Rep_Summary_all %>%  dplyr::filter(Run %in% 3) # WE WANT TO RUN ANALYSIS AND PLOT THESE DATA
Rep_Summary_R3 %>%  kbl() %>% kable_classic(full_width = F, html_font = "Cambria")


Rep_Summary_R4 <- Rep_Summary_all %>%  dplyr::filter(Run %in% 4) # for the other paper - full grow out multigenerational
Rep_Summary_R4 %>%  kbl() %>% kable_classic(full_width = F, html_font = "Cambria")


Rep_Summary_R6 <- Rep_Summary_all %>%  dplyr::filter(Run %in% 6) # WE WANT TO RUN ANALYSIS AND PLOT THESE DATA
